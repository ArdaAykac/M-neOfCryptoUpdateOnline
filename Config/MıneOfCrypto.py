...
def Rooms_system():
    ...
    def mining_loop():
        while mining_active and rooms_window.winfo_exists():
            time.sleep(1)  # Mining aktifken sadece bekler, hiçbir şey yapmaz
        if not rooms_window.winfo_exists():
            stop_mining()
...
def purchase_item(item, price):
    ...
    if Dollars >= price:
        ...
        item["Damage"] = 0  # Sadece hasar bırakıldı
...
def Payment_system():
    global Dollars, gas_bill, dat_file_path, Dollars_labels
...
def start_game_check_files_sys():
    global world_folder, dat_file_path, Dollars, gas_bill, Level, Dollars_labels
    user_profile = os.environ.get('USERPROFILE')
...
def main():
    ...
    start_game_button = tk.Button(main, text="Oyunu Başlat", command=start_game_check_files_sys, background="orange", fg="Blue", font=pixel_font)
